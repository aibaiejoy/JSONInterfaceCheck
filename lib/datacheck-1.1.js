...
(function(w){
	...
	var  checkResult = {};
	datacheck.check = function(port, kanData, KP){
		var mixData = {}, queue = [], KP = KP || "";
		
		
		$.each(port, function(k, v){
			var isKey = $.inArray(k, keys) > - 1, keyPath = !KP ? k : ( KP + "." + k );
			
			if( !isKey && $.isArray(v) ){
				mixData[k] = [];
				
				for(var i = 0, l = kanData[k].length; i < l; i++){
					mixData[k].push( datacheck.check(port[k][0], kanData[k][i], keyPath + "[" + i + "]" ) ); 
				}
			}else if( !isKey && $.isPlainObject( v ) ){
				mixData[k] = datacheck.check(port[k], kanData[k], keyPath) ; 
			//对于order数组的获取
				...
				//for(var i = 0, l = kanData[k].length; i < l; i++){
				//	mixData[k].push( datacheck.check(port[k][i], kanData[k][i]) ); 
				//}
			}else if( !isKey ) {
				if($.type( v ) === "function"){
	...
	datacheck.typeCheck = function(port, kanData){
		var dealData = this.check(port, kanData);
